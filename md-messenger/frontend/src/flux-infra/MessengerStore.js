...
var handleIncomingMessage = (state, data) => {
  ...
  else if (incomingMessage.type != 'im') {
    console.log(`Ignore unknown type: [${incomingMessage.type}]`);
    return state;
  }
  ...
  switch (incomingMessage.command) {
    ...
    case 'list':
      return state.set('messages', incomingMessage.payload);

    case 'message':
      return unshiftMessages(state, [incomingMessage.payload]);
