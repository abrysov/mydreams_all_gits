...
public class DreamLikesFragment extends BaseFragment implements EndlessScrollListener.IEndlessListener, SwipeRefreshLayout.OnRefreshListener
{
	protected static final int PAGE_SIZE = 20;
	private static final String DREAM_INFO_EXTRA_NAME = "DREAM_INFO_EXTRA_NAME";
	protected DreamLikeAdapter mAdapter;
	@Bind(R.id.list)
	RecyclerView mList;
	 ...
	 */
	@Nullable
	private BaseSpiceRequest<GetDreamLikesResponse> lastRequest;
	...
	public static Fragment getInstance(@NonNull DreamInfo item, @Nullable @StyleRes Integer theme)
	{
		...
		Fragment result = new DreamLikesFragment();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
	{
		...
		View result = inflater.inflate(R.layout.fragment_dream_likes, container, false);
		...
		mAdapter = new DreamLikeAdapter(this::handleItemClick);
	...
	private void addItems(List<LikeDto> items)
	{
		refreshLayout.setRefreshing(false);
		refreshLayout.setEnabled(!refreshLayout.isRefreshing());

		mAdapter.addAll(Stream.of(items).map(Like::new).map(DreamLikeAdapter.LikeModel::new).collect(Collectors.toList()));
	...
	private void executeRequest(RequestCriteria criteria)
	{
		...
		BaseSpiceRequest<GetDreamLikesResponse> request = RequestFactory.getDreamLikes(getDreamInfo().getId(), nextPageIndex + 1, PAGE_SIZE);
		lastRequest = request;
		getSpiceManager().execute(request, new RequestListener<GetDreamLikesResponse>()
		{
			@Override
			...
			@Override
			public void onRequestSuccess(GetDreamLikesResponse response)
			{
	...
	{
		return R.string.likes_empty_list;
	}

	private void handleItemClick(int position)
	{
		DreamLikeAdapter.LikeModel model = (DreamLikeAdapter.LikeModel) mAdapter.getItem(position);
		Fragment fragment = FlybookFragment.getInstance(model.getLike().getUser());
	...
	private void handleRequestResult(GetDreamLikesResponse response, int requestId, RequestCriteria criteria)
	{
		if (isCurrentRequest(requestId))
		{
			waitingCriteria = null;
			...
			List<LikeDto> likes = response.getLike();
			availableNextPage = likes.size() == PAGE_SIZE;
			...
			if (parent != null)
			{
				parent.setLikes(response.getBody().getLikeCollectionBody().getTotal());
			}
		}
	}
