...
@interface PostDreamViewController ()
@end


@implementation PostDreamViewController {
...
- (void)viewDidLoad {
    ...
    self.dreamTitleTextField.requiredMsg = [Helper localizedString:@"_DREAM_ADD_TITLE_NO"];
    self.dreamTitleTextField.minimum = 3;
    self.dreamTitleTextField.maximum = 160;
    self.dreamTitleTextField.limitsMsg = [Helper localizedString:@"_DREAM_ADD_TITLE_NO"];
    ...
    [self.doneSwitch addTarget:self
                        action:@selector(updateSwitchState:)
...
- (AppearenceStyle)appearenceStyle {
    if (self.editedDreamId <= 0) {
        return AppearenceStyleGREEN;
    }
...
- (NSInteger)activeMenuItem {
    return self.editedDreamId <= 0 ? 1 : 2;
}

-(BOOL)isSectionRoot {
    return self.editedDreamId <= 0 ? YES : NO;
}

- (void)updateUIForEdit {
    if (self.editedDreamId <= 0) {
        self.title = [Helper localizedString:@"_DREAM_ADD_TITLE"];
        return;
    }
    
    self.title = [Helper localizedString:@"_DREAM_EDIT_TITLE"];
    
    [self.addDreamBtn setTitle:[[Helper localizedString:@"_DREAM_EDIT_TITLE"] uppercaseString] forState:UIControlStateNormal];
    
    [self performLoading];
}

- (void)performLoading {
    [ApiDataManager dream:self.editedDreamId success:^(Dream *dream_) {
        dream = dream_;
        [self setupWithDream];
...
- (void)setupWithDream {
    if (dream.owner.id != [Helper profileUserId]) {
        [self showAlert:@"access denied"];
        self.editedDreamId = 0;
        return;
    }
    
    self.dreamTitleTextField.text = dream.name;
    self.dreamDescTextView.text = dream.description_;
    [self.dreamDescTextView setFocus:false];
    self.doneSwitch.on = dream.isdone;
    
    [Helper loadImageFrom:dream.imageUrl complete:^(NSData *imageData) {
        [self updateWithImage:[UIImage imageWithData:imageData]];
    }];
}

- (void)updateSwitchState:(id)sender {
    self.doneLabel.textColor = self.doneSwitch.on ? [UIColor blackColor] : [UIColor colorWithRed:0.54 green:0.54 blue:0.54 alpha:1.0];
}
...
- (void)updateMainImageViewWithFilter:(NSInteger)filter {
    if (currentPickedImageScaled) {
        ...
        // ставим исходную
        self.dreamPhotoImageView.image = [UIImage imageNamed:dummyImageName];
        self.dreamPhotoImageView.contentMode = UIViewContentModeScaleAspectFit;
    }
}
...
- (BOOL)validateImage {
    if (currentPickedImage == nil) {
        [self showNotification:@"_DREAM_ADD_IMAGE_NO"];
        return NO;
    }
    else {
        return YES;
...
- (BOOL)validateDreamDescr {
    if (![self.dreamDescTextView checkConstraints]) {
        [self showNotification:@"_DREAM_ADD_DESCR_NO"];
        return NO;
    }
    return YES;
}
...
- (IBAction)addDreamTouch:(id)sender {
    ...
    if (self.editedDreamId > 0) {
        UIAlertController *alert = [self showPendingAlert:[Helper localizedStringIfIsCode:@"_DREAM_EDIT_PENDING_T"] message:nil];
        
        [ApiDataManager updateDream:self.editedDreamId
                               name:self.dreamTitleTextField.text
                        description:self.dreamDescTextView.text
                              image:image
                             isdone:self.doneSwitch.on
                            success:^(void) {
            self.addDreamBtn.enabled = YES;
            [alert dismissViewControllerAnimated:YES completion:^{
                [self showAlert:NSLocalizedString(@"_DREAM_EDIT_SUCCESS", @"")];
            }];
            [self performLoading];
            [Helper clearImageForUrl:dream.imageUrl];
        } error:^(NSString *error) {
        ...
        UIAlertController *alert = [self showPendingAlert:[Helper localizedStringIfIsCode:@"_DREAM_ADD_PENDING_T"] message:nil];
        
        [ApiDataManager addDream:self.dreamTitleTextField.text
                     description:self.dreamDescTextView.text
                           image:image
                          isdone:self.doneSwitch.on
                         success:^(NSInteger id) {
            self.addDreamBtn.enabled = YES;
            [alert dismissViewControllerAnimated:YES completion:^{
                [self showAlert:NSLocalizedString(@"_DREAM_ADD_SUCCESS", @"")];
