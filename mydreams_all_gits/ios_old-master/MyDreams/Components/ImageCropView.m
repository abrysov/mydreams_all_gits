...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        IMAGE_MIN_HEIGHT = IMAGE_MIN_HEIGHT * (self.ratio > 0 ? self.ratio : 1);
        self.cropView  = [[ImageCropView alloc] initWithFrame:view blurOn:self.blurredBackground ratio:self.ratio];
...
- (id)initWithFrame:(CGRect)frame blurOn:(BOOL)blurOn ratio:(CGFloat)ratio_
{
    self = [super initWithFrame:frame];
    if (self) {
        self.blurred = blurOn;
        ratio = ratio_ > 0 ? ratio_ : 1;
        [self initViews];
    }
    return self;
}
...
- (void)initViews {
    ...
    topLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x - initialClearAreaSize,
                                                                   centerInView.y - initialClearAreaSize * ratio,
                                                                   controlPointSize)];
    ...
    bottomRightPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x + initialClearAreaSize, 
                                                                       centerInView.y + initialClearAreaSize * ratio, controlPointSize) ];
...
- (CGSize)deriveDisplacementFromDragLocation:(CGPoint)dragLocation draggedPoint:(CGPoint)draggedPoint oppositePoint:(CGPoint)oppositePoint {
    ...
    if (yDir>=0) {
        //on the top
    if(square)
        newY = oppositePoint.y - fabs(size) * ratio;
    else
