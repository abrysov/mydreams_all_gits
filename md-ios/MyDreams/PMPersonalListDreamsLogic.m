...
@interface PMPersonalListDreamsLogic ()
...
@implementation PMPersonalListDreamsLogic
@dynamic context;
...
- (void)startLogic
{
    ...
    RAC(self, localizedTitleKey) = [RACObserve(self, dreamerId) map:^id(id value) {
        return (value)? @"dreambook.list_dreams.title" : @"dreambook.list_dreams.own_dreams_title";
    }];
...
- (RACCommand *)createToFullDreamCommand
{
    ...
    return [[RACCommand alloc] initWithSignalBlock:^RACSignal *(NSNumber *postIdx) {
        ...
        [self performSegueWithIdentifier:kPMSegueIdentifierToDetailedDreamVCFromPersonalListDreamsVC context:context];
