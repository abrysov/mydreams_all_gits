...
@interface PMRecommendationsVC ()
@property (strong, nonatomic) PMRecommendationsLogic *logic;
...
@implementation PMRecommendationsVC
@dynamic logic;
...
- (void)buildSegmentedControl
{
    ...
    [self.sourceSegmentedControl insertSegmentWithTitle:NSLocalizedString(@"news.recommendations.my_recommendations", nil) atIndex:0 animated:NO];
    [self.sourceSegmentedControl insertSegmentWithTitle:NSLocalizedString(@"news.recommendations.recommendations_friends", nil) atIndex:1 animated:NO];
    self.sourceSegmentedControl.selectedSegmentIndex = 0;
}

-(IBAction)prepareForUnwindRecommendations:(UIStoryboardSegue *)segue {}
