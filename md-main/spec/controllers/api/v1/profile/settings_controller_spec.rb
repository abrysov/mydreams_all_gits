...
RSpec.describe Api::V1::Profile::SettingsController, type: :controller do
  describe 'POST #change_email' do
    ...
    context 'when sended email is blank' do
      let(:params) { { access_token: token, email: '' } }
    ...
    context 'when sended wrong email' do
      let(:params) { { access_token: token, email: '123' } }
    ...
    context 'when success update email' do
      ...
      let(:params) { { access_token: token, email: 'new@email.com' } }
  ...
  describe 'POST #change_password' do
      ...
      let(:params) do
        {
          ...
          password_confirmation: new_password, access_token: token
        }
      end
      let(:expected_message) { I18n.t('devise.confirmations.send_instructions') }
    ...
    context 'when send wrong current password' do
      let(:params) do
        {
          current_password: '123', password: new_password,
          password_confirmation: new_password, access_token: token
        }
      end
    ...
    context 'when send wrong password confirmation' do
      let(:params) do
        {
          current_password: dreamer.password, password: new_password,
          password_confirmation: '123', access_token: token
        }
      end
    ...
    context 'when send without password' do
      let(:params) do
        {
          current_password: dreamer.password,
          password_confirmation: new_password, access_token: token
        }
      end
