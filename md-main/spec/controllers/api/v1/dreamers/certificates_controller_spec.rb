...
RSpec.describe Api::V1::Dreamers::CertificatesController, type: :controller do
  ...
  describe 'GET #index' do
    ...
    context 'when get all certificates' do
      before { get :index, access_token: token, dreamer_id: dreamer.id }
    ...
    context 'when get buyed certificates' do
      before { get :index, access_token: token, paid: true, dreamer_id: dreamer.id }
    ...
    context 'when get gifted certificates' do
      before { get :index, access_token: token, gifted: true, dreamer_id: dreamer.id }
    ...
    context 'when get certificates ordered by created date' do
      before { get :index, access_token: token, new: true, dreamer_id: dreamer.id }
    ...
    context 'when get certificates ordered by launches count(status)' do
      before { get :index, access_token: token, dreamer_id: dreamer.id, launches: true }
